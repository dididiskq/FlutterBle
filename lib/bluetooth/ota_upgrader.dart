...
class OtaUpgrader {
  ...
  // 升级状态流
  final StreamController<UpgradeStatusData> _upgradeStatusController =
      StreamController.broadcast();
  Stream<UpgradeStatusData> get upgradeStatusStream =>
      _upgradeStatusController.stream;
  ...
  /// 选择升级文件
  Future<void> selectFirmwareFile(
    ...
    _upgradeStatusController
        .add(UpgradeStatusData(UpgradeStatus.idle, 0, '固件文件已选择'));
  }
  ...
  Future<void> startUpgrade({required int otaSelection}) async {
    ...
    try {
      ...
      _upgradeStatusController
          .add(UpgradeStatusData(UpgradeStatus.checkingUpdate, 0, '开始升级...'));

      // 1. 连接参数更新
      await _connectionUpdate();
      _upgradeStatusController
          .add(UpgradeStatusData(UpgradeStatus.checkingUpdate, 10, '连接参数更新完成'));

      // 2. MTU 更新
      await _mtuUpdate();
      _upgradeStatusController
          .add(UpgradeStatusData(UpgradeStatus.checkingUpdate, 20, 'MTU 更新完成'));

      // 3. 版本请求
      await _versionRequest();
      _upgradeStatusController
          .add(UpgradeStatusData(UpgradeStatus.checkingUpdate, 30, '版本请求完成'));
      ...
      if (_otaSelection == 4) {
        ...
        _upgradeStatusController
            .add(UpgradeStatusData(UpgradeStatus.upgrading, 40, '已跳转至镜像更新程序'));
        // 后续流程由镜像更新程序处理
        
        // 处理单 Bank 升级（ota_selection == 4）
        ...
        _upgradeStatusController.add(
            UpgradeStatusData(UpgradeStatus.checkingUpdate, 40, '创建设置传输完成'));

        // 5. 发送设置数据
        if (_dfuSettingData != null) {
          await _sendSettingData(_dfuSettingData!);
          _upgradeStatusController.add(
              UpgradeStatusData(UpgradeStatus.downloading, 50, '设置数据发送完成'));
        } else {
          print('[OTA] 警告：设置数据为空，跳过发送设置数据');
          _upgradeStatusController.add(
              UpgradeStatusData(UpgradeStatus.downloading, 50, '设置数据为空，跳过发送'));
        }

        // 6. & 7. 创建镜像传输和发送镜像数据（合并为一个步骤，与Java实现一致）
        await _sendImageData();
        _upgradeStatusController
            .add(UpgradeStatusData(UpgradeStatus.downloading, 60, '开始发送镜像数据'));
        _upgradeStatusController
            .add(UpgradeStatusData(UpgradeStatus.upgrading, 90, '镜像数据发送完成'));

        // 8. 验证新镜像
        await _validateNewImage();
        _upgradeStatusController
            .add(UpgradeStatusData(UpgradeStatus.upgrading, 95, '镜像验证完成'));

        // 9. 激活新镜像
        await _activateNewImage();
        _upgradeStatusController
            .add(UpgradeStatusData(UpgradeStatus.completed, 100, '升级完成'));
        
        // 处理单 Bank 升级（ota_selection == 3）
        if (_otaSelection == 3) {
          await _handleSingleBankUpgrade();
        }
      }
      ...
      await _bleController.disableOtaMode();
      _upgradeStatusController
          .add(UpgradeStatusData(UpgradeStatus.failed, 0, '升级失败: $error'));
      rethrow;
    }
  }
  ...
  Future<void> _connectionUpdate() async {
    ...
    print(
        '[OTA] 命令详情: intervalMin=$intervalMin, intervalMax=$intervalMax, latency=$latency, timeout=$timeout');
    ...
    try {
      ...
      if (response.length >= 2) {
        ...
        if (otaError > 0) {
          ...
          _upgradeStatusController.add(UpgradeStatusData(
              UpgradeStatus.failed, 0, '连接参数更新失败: 错误代码 $otaError'));
      ...
      _upgradeStatusController
          .add(UpgradeStatusData(UpgradeStatus.failed, 0, '连接参数更新失败: $error'));
      rethrow;
    }
  }
    ...
    try {
      ...
      if (response.length >= 2) {
        ...
        if (otaError > 0) {
          ...
          _upgradeStatusController.add(UpgradeStatusData(
              UpgradeStatus.failed, 0, 'MTU更新失败: 错误代码 $otaError'));
      ...
      _upgradeStatusController
          .add(UpgradeStatusData(UpgradeStatus.failed, 0, 'MTU更新失败: $error'));
      rethrow;
    }
  }
  ...
  Future<void> _versionRequest() async {
    ...
    int imageUpdateBinSize =
        imageUpdateFile.existsSync() ? await imageUpdateFile.length() : 0;
    ...
    print(
        '[OTA] 发送版本请求命令: ${data.map((b) => '0x${b.toRadixString(16).padLeft(2, '0')}').join(' ')}');
    print(
        '[OTA] 版本请求参数: app1BinSize=$app1BinSize, app2BinSize=$app2BinSize, imageUpdateBinSize=$imageUpdateBinSize, imageUpdateVersion=0x${imageUpdateVersion.toRadixString(16)}');
    ...
    try {
      ...
      print(
          '[OTA] 收到版本请求响应: ${response.map((b) => '0x${b.toRadixString(16).padLeft(2, '0')}').join(' ')}');
      ...
      if (response.length >= 14) {
        ...
        if (response[0] != OTA_CMD_VERSION) {
          print(
              '[OTA] 版本请求失败: 响应命令ID不匹配，期望 0x${OTA_CMD_VERSION.toRadixString(16)}，实际 0x${response[0].toRadixString(16)}');
        ...
        print(
            '[OTA] image_update_version: 0x${_imageUpdateVersion.toRadixString(16)}');
        ...
        if (_otaSelection >= 1 && _otaSelection <= 4) {
          ...
          _upgradeStatusController.add(UpgradeStatusData(
              UpgradeStatus.failed, 0, '版本请求失败: 无效的OTA选择 $_otaSelection'));
      ...
      _upgradeStatusController
          .add(UpgradeStatusData(UpgradeStatus.failed, 0, '版本请求失败: $error'));
      rethrow;
    }
  }
      ...
      if (index < confStrs.length) {
        ...
        if (parts.length > 1) {
          ...
          if (versionStr.length >= 11) {
            String hexStr = versionStr.substring(3, 11);
            // 转换为整数 - 与Java实现一致
            int version = int.parse(hexStr, radix: 16);
            print(
                '[OTA] 解析到IMAGE_UPDATE_VERSION: 0x${version.toRadixString(16)}');
            return version;
          } else {
            print('[OTA] IMAGE_UPDATE_VERSION字符串长度不足，格式: $versionStr');
            return 0x01000000;
          }
        }
      }
  ...
  Future<void> _createOtaSettingTransfer(int dfuSettingSize) async {
    ...
    try {
      ...
      _upgradeStatusController
          .add(UpgradeStatusData(UpgradeStatus.failed, 0, '创建设置传输失败: $error'));
      rethrow;
    }
  }
    ...
    try {
      ...
      if (response.length >= 2) {
        ...
        if (otaError > 0) {
          ...
          _upgradeStatusController.add(UpgradeStatusData(
              UpgradeStatus.failed, 0, '发送设置数据失败: 错误代码 $otaError'));
      ...
      _upgradeStatusController
          .add(UpgradeStatusData(UpgradeStatus.failed, 0, '发送设置数据失败: $error'));
      rethrow;
    } finally {
  ...
  Future<void> _createOtaImageTransfer({
    ...
    try {
      ...
      print(
          '[OTA] 发送创建镜像传输命令: ${data.map((b) => '0x${b.toRadixString(16).padLeft(2, '0')}').join(' ')}');

      await _bleController.writeOtaCommand(data);

      print('[OTA] 创建镜像传输命令写入成功');
    } catch (error) {
      print('[OTA] 创建镜像传输失败: $error');
      _upgradeStatusController
          .add(UpgradeStatusData(UpgradeStatus.failed, 0, '创建镜像传输失败: $error'));
      rethrow;
    }
  }

  /// 7. 发送镜像数据
  /// [prn] - Packet Reorder Number，每次最多发送的字节数（默认2048，与Java实现一致）
  Future<void> _sendImageData({int prn = 2048}) async {
    if (_extractedFirmwarePath == null) {
      throw Exception('解压固件路径未设置');
    }

    // 根据OTA选择动态选择固件文件 - 与Java实现一致
    String firmwarePath;
    if (_otaSelection == 1) {
      firmwarePath = '$_extractedFirmwarePath/APP1.bin';
    } else if (_otaSelection == 2) {
      firmwarePath = '$_extractedFirmwarePath/APP2.bin';
    } else if (_otaSelection == 3) {
      firmwarePath = '$_extractedFirmwarePath/ImageUpdate.bin';
    } else {
      throw Exception('无效的OTA选择: $_otaSelection，无法确定固件文件');
    }
    ...
    print('[OTA] 开始发送镜像数据，固件路径: $firmwarePath, 总长度: $firmwareSize, PRN: $prn');

    // 使用PRN作为最大分块大小，与Java实现一致
    final int maxChunkSize = prn;

    // 一次性打开文件，避免频繁 open/close
    ...
    try {
      while (offset < firmwareSize) {
        final int remaining = firmwareSize - offset;
        final int chunkSize =
            remaining > maxChunkSize ? maxChunkSize : remaining;
        ...
        print(
            '[OTA] 准备发送镜像数据块: offset=$offset, size=${chunk.length}, crc=0x${chunkCrc.toRadixString(16)}');

        // 每次新建 Completer 和 Subscription（简化版，性能稍低但安全）
        final Completer<void> blockCompleter = Completer<void>();
        late StreamSubscription<List<int>> blockSub;

        blockSub = _bleController.notificationStream.listen((data) {
          print(
              '[OTA] 收到镜像数据响应: ${data.map((b) => '0x${b.toRadixString(16).padLeft(2, '0')}').join(' ')}');
          if (data.isNotEmpty && data[0] == OTA_CMD_CREATE_OTA_IMAGE) {
            final err = data[1];
            if (err > 0) {
              print('[OTA] 镜像数据传输失败: 错误代码 $err');
              if (!blockCompleter.isCompleted) {
                blockCompleter.completeError(Exception('块传输失败: $err'));
              }
            } else {
              print('[OTA] 镜像数据传输成功');
              if (!blockCompleter.isCompleted) {
          ...
          if (!blockCompleter.isCompleted) {
            blockCompleter.completeError(error);
          }
        });
        ...
        try {
          ...
          // 2. 发送数据块（使用 writeWithoutResponse）
          print('[OTA] 开始发送镜像数据块...');
          ...
          // 3. 等待响应（5秒足够）
          print('[OTA] 等待设备响应...');
          await blockCompleter.future.timeout(const Duration(seconds: 5));
          print('[OTA] 收到设备响应确认');

          // 更新进度
          totalSent += chunk.length;
          final int progress =
              firmwareSize > 0 ? (totalSent * 100) ~/ firmwareSize : 0;
          final int adjustedProgress =
              60 + ((progress * 30) ~/ 100); // 60% ～ 90%
          _upgradeStatusController.add(UpgradeStatusData(
            UpgradeStatus.downloading,
            adjustedProgress,
  ...
  void _testCrc32() {
    ...
    print(
        '[CRC32测试] 输入: ${testData.map((e) => '0x${e.toRadixString(16).padLeft(2, '0')}').join(', ')}');
    print(
        '[CRC32测试] CRC32结果: 0x${crcResult.toRadixString(16).padLeft(8, '0')}');
  }
  ...
  Future<void> _validateNewImage() async {
    ...
    try {
      ...
      final Uint8List command =
          Uint8List.fromList([OTA_CMD_VALIDATE_OTA_IMAGE]);
  ...
  Future<void> _activateNewImage() async {
    ...
    try {
      // 更新状态为90%进度
      _upgradeStatusController
          .add(UpgradeStatusData(UpgradeStatus.downloading, 90, '正在激活新镜像...'));

      // 构建激活命令
      final Uint8List command =
          Uint8List.fromList([OTA_CMD_ACTIVATE_OTA_IMAGE]);
      ...
      try {
        ...
        final response = await completer.future
            .timeout(const Duration(seconds: 5)); // 调整超时时间
  ...
  Future<void> cancelUpgrade() async {
    _upgradeStatusController
        .add(UpgradeStatusData(UpgradeStatus.idle, 0, '升级已取消'));
  }

  /// 处理单 Bank 升级（ota_selection == 3/4）
  /// 参考Java代码：断开连接 → 等待设备重启 → 重连新地址（MAC+1）→ 再次升级
  Future<void> _handleSingleBankUpgrade() async {
    print('[OTA] 开始处理单 Bank 升级，ota_selection: $_otaSelection');
    
    try {
      // 获取当前设备ID
      final String currentDeviceId = _bleController.connectedDevice!.id;
      print('[OTA] 当前设备ID: $currentDeviceId');
      
      // Step 1: 断开当前连接
      print('[OTA] 断开当前连接，等待设备重启...');
      ...
      // Step 3: 计算新的MAC地址（MAC+1）
      final String newMacAddress = _incrementMacAddress(currentDeviceId);
      print('[OTA] 计算新MAC地址: $newMacAddress');
      
      // Step 4: 重连到新MAC地址
      print('[OTA] 重连到新MAC地址: $newMacAddress');
      _upgradeStatusController.add(UpgradeStatusData(
        UpgradeStatus.upgrading,
        96,
        '重连到新设备地址...',
      ...
      // Step 5: 再次发送镜像数据（第二个升级周期）
      print('[OTA] 开始第二次升级周期，发送镜像数据...');
      _upgradeStatusController.add(UpgradeStatusData(
        UpgradeStatus.upgrading,
        97,
        '开始第二次升级周期...',
      ...
      // 使用PRN=2048发送镜像数据
      await _sendImageData(prn: 2048);
      
      print('[OTA] 单 Bank 升级处理完成');
    } catch (e) {
      print('[OTA] 处理单 Bank 升级失败: $e');
      _upgradeStatusController.add(UpgradeStatusData(
        UpgradeStatus.failed,
        0,
        '单 Bank 升级处理失败: $e',
  ...
  /// 将MAC地址加1（用于单 Bank 升级后的重连）
  /// 输入格式: AA:BB:CC:DD:EE:FF 或 AABBCCDDEEFF
  /// 输出格式: 与输入格式一致
  String _incrementMacAddress(String macAddress) {
    print('[OTA] 原始MAC地址: $macAddress');
    
    // 移除MAC地址中的分隔符
    String cleanMac = macAddress.replaceAll(':', '').replaceAll('-', '').toUpperCase();
    print('[OTA] 清理后的MAC地址: $cleanMac');
    
    // 检查MAC地址长度
    if (cleanMac.length != 12) {
      print('[OTA] MAC地址格式无效，返回原始地址');
      return macAddress;
    }
    
    try {
      // 将MAC地址转换为十进制数值
      BigInt macValue = BigInt.parse(cleanMac, radix: 16);
      print('[OTA] MAC地址十进制值: $macValue');
      
      // MAC地址加1
      BigInt newMacValue = macValue + BigInt.one;
      print('[OTA] 新MAC地址十进制值: $newMacValue');
      
      // 转换回十六进制字符串，确保12位长度
      String newCleanMac = newMacValue.toRadixString(16).toUpperCase().padLeft(12, '0');
      print('[OTA] 新清理后的MAC地址: $newCleanMac');
      
      // 恢复原始格式的分隔符
      String newMacAddress;
      if (macAddress.contains(':')) {
        // 格式: AA:BB:CC:DD:EE:FF
        newMacAddress = '${newCleanMac.substring(0, 2)}:${newCleanMac.substring(2, 4)}:${newCleanMac.substring(4, 6)}:${newCleanMac.substring(6, 8)}:${newCleanMac.substring(8, 10)}:${newCleanMac.substring(10, 12)}';
      } else if (macAddress.contains('-')) {
        // 格式: AA-BB-CC-DD-EE-FF
        newMacAddress = '${newCleanMac.substring(0, 2)}-${newCleanMac.substring(2, 4)}-${newCleanMac.substring(4, 6)}-${newCleanMac.substring(6, 8)}-${newCleanMac.substring(8, 10)}-${newCleanMac.substring(10, 12)}';
      } else {
        // 格式: AABBCCDDEEFF
        newMacAddress = newCleanMac;
      }
      
      print('[OTA] 最终新MAC地址: $newMacAddress');
      return newMacAddress;
    } catch (e) {
      print('[OTA] 计算新MAC地址失败: $e，返回原始地址');
      return macAddress;
    }
  }
  ...
  Future<void> _jumpToImageUpdate() async {
    ...
    try {
      // 发送跳转命令
      final Uint8List jumpCommand =
          Uint8List.fromList([OTA_CMD_JUMP_IMAGE_UPDATE]);

      // 更新状态
      _upgradeStatusController
          .add(UpgradeStatusData(UpgradeStatus.upgrading, 35, '正在跳转至镜像更新...'));
      ...
      if (response.length >= 2) {
        ...
        if (otaError > 0) {
          ...
          _upgradeStatusController.add(UpgradeStatusData(
              UpgradeStatus.failed, 0, '跳转至镜像更新失败: 错误代码 $otaError'));
      ...
      _upgradeStatusController
          .add(UpgradeStatusData(UpgradeStatus.failed, 0, '跳转至镜像更新失败: $error'));
      rethrow;
    }
  }
}
